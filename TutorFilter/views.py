...
def filter_tutors(request):
    ...
    if form.is_valid():
        ...
        if form.cleaned_data["preferred_mode"] and form.cleaned_data["preferred_mode"]!="..":
            users = users.filter(
    ...
    return render(
        request, "TutorFilter/filter_results.html", {"form": form, "users": users,'MEDIA_URL': settings.MEDIA_URL,}
    )
